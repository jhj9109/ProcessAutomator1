...
from constants import FIRST_ITEM_ROW_INDEX, SUMMARY_FIRST_ITEM_ROW_INDEX, IMAGE_CELL_HEIGHT_PT, IMAGE_CELL_WIDTH_PT_6
...
def set_column_width(ws, min_row, max_row, col):
    ...
    logest_column_length = get_longest_column_length(
        ws, min_row, max_row, col)  # 2: 여유공간
    if col == 3:
        # 단지명
...
def create_new_summary_xlsx(아파트목록, 엑셀파일명):
    ...
    for idx, 아파트객체 in enumerate(아파트목록):
        i = SUMMARY_FIRST_ITEM_ROW_INDEX + idx
        # 추가
    ...
    i = SUMMARY_FIRST_ITEM_ROW_INDEX + len(아파트목록)
    ws.merge_cells(f'A{i}:C{i}')
    ws[f'A{i}'].value = "합계"
    ws[f'D{i}'].value = f"=SUM(D{SUMMARY_FIRST_ITEM_ROW_INDEX}:D{i-1})"
    ws[f'E{i}'].value = f"=SUM(E{SUMMARY_FIRST_ITEM_ROW_INDEX}:E{i-1})"
    ws[f'F{i}'].value = f"=E{i} / D{i}"
    set_percent_style(ws[f'F{i}'])
